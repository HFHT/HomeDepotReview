...
// ---------------------------------------------------------------------------
// Text-extraction request/response types (analyzeTextReceipt)
// ---------------------------------------------------------------------------

/**
 * Hard cap on the number of characters accepted for the tabularized PDF text
 * extraction endpoint. Intentionally generous for now — expect to ratchet
 * this down once real-world large-receipt behavior has been observed.
 */
export const MAX_TEXT_RECEIPT_CHARS = Number(
  process.env.MAX_TEXT_RECEIPT_CHARS ?? 750_000
);

export const AnalyzeTextReceiptRequestSchema = z.object({
  text: z
    .string()
    .min(1, "text must not be empty")
    .max(
      MAX_TEXT_RECEIPT_CHARS,
      `text must not exceed ${MAX_TEXT_RECEIPT_CHARS} characters`
    ),
});

export type AnalyzeTextReceiptRequest = z.infer<
  typeof AnalyzeTextReceiptRequestSchema
>;

/**
 * Same shape as ReceiptAnalysisResponse, minus image_results — there are no
 * images/pages to grade for quality/readability when the input is already
 * machine-extracted text rather than a scanned image or PDF document block.
 */
export type TextReceiptAnalysisResponse = {
  supplier: string | null;
...
export const TextReceiptAnalysisResponseSchema = z.object({
  supplier: z.string().nullable(),
  receipt_type: z.string().nullable(),
  store: z.string().nullable(),
  store_number: z.string().nullable(),
